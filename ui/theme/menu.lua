...
theme.Label = function(text, opt, x, y, w, h)
  ...
  lg.printf(text or opt.text, font, x + 2, y, w - 4, opt.align or "left")
end
...
theme.Button = function(text, opt, x, y, w, h)
  ...
  if not opt.noBox then
    theme.drawBox(x + opt.x, y + opt.y, w + opt.w, h + opt.h, c, (opt.x ~= 0 and opt.r or -opt.x)*3, opt.alpha)
  end
  ...
  if type(text) == "string" then
    ...
    lg.draw(text, x + opt.x, y, 0, w/tw, h/th)
  end
end
...
theme.Input = function(input, opt, x, y, w, h)
  ...
  lg.push("all")
    ...
    -- text
    lg.setColor(opt.color and opt.color.normal and opt.color.normal.fg or theme.color.normal.fg)
    local masked_or_normal = input.text
    if input.masked then
      masked_or_normal = string.rep("*", utf8.len(input.text))
    end
    lg.print(masked_or_normal, font, x, y + (h - th) / 2)
    -- candidate text
...
theme.Shape = function(color, opt, x, y, w, h)
  theme.drawBox(x, y, w, h, color, opt.r)
end
